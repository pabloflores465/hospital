...
<template>
  <div
    class="p-2 flex w-full items-center bg-white border-b-2 border-blue-300 shadow-2xl"
  >
    <NuxtLink to="/" class="w-16 h-16 me-2">
      <img
        src="public/logo.webp"
        alt="Description"
        class="rounded-full border-gray-300 object-cover w-full h-full"
      />     
    </NuxtLink>
    <Switch :onchange="(event:Event) => {
      edit = (event.target as HTMLInputElement)?.checked;
      console.log(edit)
    }" />
    <NuxtLink to="/login">
      <Button layout="pe-2">
